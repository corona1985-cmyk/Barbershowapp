                        ...
                        return (
                            ...
                            >
                                ...
                                <div className="p-6 flex flex-col flex-1">
                                    ...
                                    {pos.about?.trim() && (
                                        <p className="text-sm text-slate-600 mb-3 line-clamp-2">{pos.about.trim()}</p>
                                    )}
                                    {pos.highlights && pos.highlights.length > 0 && (
                                        <div className="flex flex-wrap gap-1.5 mb-3">
                                            {pos.highlights.slice(0, 4).map((h) => (
                                                <span key={h} className="px-2 py-0.5 rounded-md bg-slate-100 text-slate-600 text-[11px] font-medium">
                                                    {h}
                                                </span>
                                            ))}
                                        </div>
                                    )}
                                    {pos.certifications && pos.certifications.length > 0 && (
                                        <p className="text-xs text-amber-800 font-semibold mb-3">
                                            {t('profile.certsCount', { count: pos.certifications.length })}
                                        </p>
                                    )}
